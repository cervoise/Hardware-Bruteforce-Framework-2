...
class Attack:
	def __init__(self, path_to_pattern, path_to_logins = "", path_to_passwords = "", path_to_screenshots = "", no_screenshots=False):
		...
		if path_to_screenshots != '' and self.pattern.useScreenshot() is False:
			print "A screenshots folder is specify, but there is not screenshot command in pattern file."
			sys.exit(1)
		if path_to_logins != '' and self.pattern.useLogin() is False:
			print "A logins file is specify, but there is not login command in pattern file."
			sys.exit(1)
		elif path_to_logins == '' and self.pattern.useLogin() is True:
			print "A login command is in pattern file, but there is not logins file specify."
			sys.exit(1)
			
		if path_to_passwords != '' and self.pattern.usePassword() is False:
			print "A passwords file is specify, but there is not password command in pattern file."
			sys.exit(1)
		elif path_to_passwords == '' and self.pattern.usePassword() is True:
			print "A password command is in pattern file, but there is not passwords file specify."
			sys.exit(1)
